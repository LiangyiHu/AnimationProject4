...
glm::vec4 getQuaternionFromAngleAndAxle(GLfloat angle, glm::vec3 axle);
...
glm::mat4 getRotationMatrixDirectionAndUp(glm::vec3 position, glm::vec3 direction);
...
glm::vec4 getQuaternionFromAngleAndAxle(GLfloat angle, glm::vec3 axle) {
	...
	returnQuat.x = axle.x*glm::sin(angle/2);
	returnQuat.y = axle.y*glm::sin(angle / 2);
	returnQuat.z = axle.z*glm::sin(angle / 2);
	return returnQuat;
}

GLfloat getAngleFromQuat(glm::vec4 inputQuat) {
	glm::vec4 reshapedQuat = glm::normalize(inputQuat);
	GLfloat returnAngle = 2*glm::acos(reshapedQuat.w);
	return returnAngle;
}
glm::vec3 getAxleFromQuat(glm::vec4 inputQuat) {
	glm::vec3 returnAxle;
	glm::vec4 reshapedQuat = glm::normalize(inputQuat);
	if (getAngleFromQuat(reshapedQuat)==0) {
		return glm::vec3(0.0f,1.0f,0.0f);
	}
	returnAxle.x = reshapedQuat.x / (glm::sqrt(1 - glm::pow(reshapedQuat.w,2)));
	returnAxle.y = reshapedQuat.y / (glm::sqrt(1 - glm::pow(reshapedQuat.w, 2)));
	returnAxle.z = reshapedQuat.z / (glm::sqrt(1 - glm::pow(reshapedQuat.w, 2)));
	return returnAxle;
}
...
//This function use euler angles, assume y axle is up vector
glm::mat4 getRotationMatrixDirectionAndUp(glm::vec3 position, glm::vec3 direction) {
	glm::mat4 returnMatrix;
	...
	if (direction.x== 0&& direction.z == 0) {
		eulerAngles.x=0.0f;
		eulerAngles.y = 0.0f;
		eulerAngles.z = direction.y>=0.0f?90.0f:-90.0f;
	}
	else if(direction.x == 0)
	{
		eulerAngles.x = 0.0f;
		eulerAngles.y = direction.z>=0? -90.0f:90.0f;
		eulerAngles.z = glm::degrees(glm::atan(direction.y/direction.z));
	}
	else if (direction.z == 0)
	{
		eulerAngles.x = 0.0f;
		eulerAngles.y = direction.x >= 0 ? 0.0f : 180.0f;
		eulerAngles.z = direction.x>=0?glm::degrees(glm::atan(direction.y / direction.x)): -glm::degrees(glm::atan(direction.y / direction.x));
	}
	else {
		GLfloat yVertical = glm::sqrt(glm::pow(direction.x, 2) + glm::pow(direction.z, 2));
		eulerAngles.x = 0.0f;
		eulerAngles.y = direction.z>0? (direction.x>0? -glm::degrees(glm::atan(direction.z / direction.x)): glm::degrees(glm::atan(direction.z / -direction.x))-180.0f):(direction.x>0 ? glm::degrees(glm::atan(direction.z / -direction.x)) :180.0f- glm::degrees(glm::atan(direction.z / direction.x)));
		eulerAngles.z = glm::degrees(glm::atan(direction.y / yVertical));
	}
	//glm::transpose(getFinalMatrixEuler(position, glm::vec3(eulerAngles.x, eulerAngles.y, eulerAngles.z)));
	returnMatrix = glm::translate(glm::mat4(),position);
	returnMatrix = glm::rotate(returnMatrix, glm::radians(eulerAngles.x), glm::vec3(1.0f, 0.0f, 0.0f));
	returnMatrix = glm::rotate(returnMatrix, glm::radians(eulerAngles.y), glm::vec3(0.0f, 1.0f, 0.0f));
	returnMatrix = glm::rotate(returnMatrix,glm::radians(eulerAngles.z),glm::vec3(0.0f,0.0f,1.0f));
	return returnMatrix;
}
